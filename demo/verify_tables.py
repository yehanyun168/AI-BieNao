...
LAYERS = {
    ...
    'paths.py':           0,   # 2026-09-16（协作者张博文交付版引入）：运行时数据目录
                               #       解析（源码态=demo/，冻结态=%APPDATA%/AI-BieNao）
                               #       + 旧版数据迁移。仅依赖标准库 os/shutil/sys，
                               #       零项目依赖，定 L0 与 preferences 同级
    # L1 基础层
...
'ui_v4.py':          2097,    # 2026-09-16 六度登记（原 2058）：对齐协作者张博文最新
                                  #    交付版（AI-BieNao.zip）—— 新增文字自适应助手：
                                  #    _sync_text_bounds 按 texture 实时回写可用宽度、
                                  #    页面返回键 _fit_back_button、动作按钮
                                  #    _fit_action_button（texture_update 后重排版），
                                  #    修复设置项 / 图例 / 返回键文案超宽被裁（+39 行）
                                  #    （原 2058 = 2026-09-14 五度登记：检视卡/科技描述中文化
    ...
    'main.py':           1675,    # 2026-09-17 九度登记（原 1650）：窗口图标改封面图标
                                  ...
                                  #    （原 1543 = 2026-09-14 五度登记：开场动画收尾停
    ...
    'i18n.py':           1416,    # 2026-09-17 九度登记（原 1414）：新增返回主菜单确认框 ZH 三词条
                                  ...
                                  #     精简为单行）
    'ui_hud.py':          964,    # 2026-09-16 五度登记（原 945）：对齐协作者张博文
                                  #    最新交付版 —— 新增 _fit_content 内容自适应
                                  #    （图例 Legend 动态宽度 + 顶栏预测前缀 + 区域
                                  #    HUD 偏移重排），修复窄窗口下文字出框（+19 行）
                                  #    （原 945 = 2026-09-16 四度登记：对齐协作者张博文
                                  #    交付版（zip）后的实测值；本地手动缩放控件
                                  #    btn_minus/btn_plus/zoom_row 已按其版本移除，当前
                                  #    行数为交付版自身总量（+23 行）
                                  #    （原 922 = 2026-09-14 三度登记：右侧指令栏 RailBar
